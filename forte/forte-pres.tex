...
\note{
  ...
  This work is about analyzing distributed systems programs
  from a global perspective with a novel extraction technique,
  that works when we only have access to the codebase.
}
...
\node[box] (step1) {Take the\\file input\\and perform\\some simple\\analysis};
...
\node[box, right=of step2] (step3) {Create a\\global-view\\using the\\local-views};
