...
the message (additional condition: $\forall i,j \in I. q_i=q_j$). This restriction allow 
some undesired behavior when we observe the system from an implementability point of view.
...
\begin{equation}
L ::=  \! \langle p,S \rangle.T \mid ? (p,U).T \mid \&(p, \{ l_i: T_i \}_{i\in I}) \mid \oplus(p, \{ l_i: T_i \}_{i\in I}) \mid \mu t. L \mid t \mid \text{end}
\end{equation}

where:
\begin{itemize}
    \item $!$ means the sending of a value,
    \item $?$ means the receiving of a message,
    \item $\&$ denotes the selection operation,
    \item $\oplus$ represents the branching operator,
