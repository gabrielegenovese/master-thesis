...
\subsubsection{Feasible Execution}
...
A p2p-MSC is an MSC $M = (E, →, \lhd, \lambda)$ where, for any two send events $s$ 
...
An MSC $M = (E, →, \lhd, \lambda)$ is causally ordered if, for any two send $s$ and 
...
An MSC $M = (E, →, \lhd, \lambda)$ is a \verb|mb|-MSC if it has a linearization 
...
An MSC $M = (E, →, \lhd, \lambda)$ is a \verb|onen|-MSC if it has a linearization 
$\rightsquigarrow$ where, for any two send events $s$ and $s'$, such 
...
An MSC $M = (E, →, \lhd, \lambda)$ is a \verb|nn|-MSC if it has a linearization 
$\rightsquigarrow$ where, for any two send events $s$ and $s'$, such 
...
An MSC $M = (E, →, \lhd, \lambda)$ is an \verb|rsc|-MSC if it has no unmatched 
