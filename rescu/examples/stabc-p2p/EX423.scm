...
state 18 :
to 21 : when true , 2 ! LBREPLY;

state 19 :
to 21 : when true , 2 ! LBREPLY;

state 17 :
to 18 : when true , 5  ? VM1RES;
to 17 : when true , 5  ? VM1INFO;

state 4 :
to 7 : when true , 2 ! LBREQUEST;

state 14 :
to 19 : when true , 6  ? VM2RESULT;
...
to 20 : when true , 3  ? LBRESULT;

state 12 :
to 16 : when true , 1 ! VM2LOGIN;
to 15 : when true , 2 ! LBREPLY;

state 8 :
to 13 : when true , 1 ! VM2LOGIN;
to 12 : when true , 5  ? VM1RES;
to 11 : when true , 5  ? VM1INFO;
...
state 21 :
to 24 : when true , 3  ? LBRESULT;

state 10 :
to 14 : when true , 1 ! VM2REQUEST;
to 13 : when true , 4 ! VM1REQ;
...
to 22 : when true , 7 ! CLIENTRESULT;
