...
state 0 :
to 1 : when true , 2  ? BREQUEST;

state 11 :
to 6 : when true , 3 ! BACK;

state 9 :
to 10 : when true , 0 ! SBUY;

state 4 :
to 5 : when true , 3 ! BREPLY;

state 5 :
to 12 : when true , 2  ? BSTOP;
to 9 : when true , 2  ? BPURCHASE;
to 7 : when true , 2  ? BREQUEST;
...
state 1 :
to 2 : when true , 3  ? BREPLY;

state 0 :
to 1 : when true , 2 ! BREQUEST;

state 4 :
to 5 : when true , 3  ? BACK;
...
state 2 :
to 4 : when true , 2 ! BPURCHASE;
to 3 : when true , 2 ! BSTOP;
to 1 : when true , 2 ! BREQUEST;
