...
state 1 :
to 2 : when true , 0 ! PPAYMENTTYPE;

state 0 :
to 1 : when true , 0 ! PFLIGHTSELECTED;

state 4 :
to 5 : when true , 3  ? PTXOK;
to 6 : when true , 0 ! PPAYMENT;

state 5 :
to 7 : when true , 0 ! PPAYMENT;

state 3 :
to 6 : when true , 0 ! PCLIENTID;

state 7 :
to 8 : when true , 3  ? PTICKETINFO;

state 8 :

state 2 :
to 4 : when true , 0 ! PCLIENTID;
to 3 : when true , 0 ! PPAYMENT;

state 6 :
to 7 : when true , 3  ? PTXOK;
...
state 3 :
to 5 : when true , 0  ? PCLIENTID;

state 16 :

state 14 :
to 15 : when true , 3 ! PTXOK;
