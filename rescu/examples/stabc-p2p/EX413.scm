...
state 1 :
to 2 : when true , 3  ? DMFAILED1;

state 0 :
to 1 : when true , 0 ! DMINSTANTIATE1;

state 3 :
to 1 : when true , 0 ! DMALERTFAILURE;

state 2 :
to 3 : when true , 0 ! DMINSTANTIATE1;
...
state 10 :
to 4 : when true , 0  ? DMALERTFAILURE;
...
state 5 :
to 6 : when true , 0  ? DMINSTANTIATE1;

state 4 :
to 5 : when true , 3 ! DMFAILED1;
...
to 9 : when true , 0  ? DMALERTFAILURE;

state 6 :
to 8 : when true , 1 ! VM1RESETINSTANCE;
to 7 : when true , 0  ? DMALERTFAILURE;
