...
state 1 :
to 2 : when true , 0  ? CGETITEM2;

state 0 :
to 1 : when true , 2 ! CGETITEM1;

state 4 :
to 6 : when true , 2 ! CBUY;

state 5 :

state 3 :
to 5 : when true , 2 ! CCANCEL;
...
state 6 :
to 5 : when true , 0  ? CCONFIRMATION;
...
state 4 :
to 5 : when true , 0 ! CGETITEM2;

state 5 :
to 6 : when true , 2  ? CBUY;
to 7 : when true , 2  ? CCANCEL;
...
state 15 :
to 11 : when true , 0 ! CCONFIRMATION;

state 10 :
to 13 : when true , 2  ? SDONE;
to 12 : when true , 0 ! CCONFIRMATION;
...
to 14 : when true , 0 ! CCONFIRMATION;
