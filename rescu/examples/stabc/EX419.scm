...
state 18 :
to 19 : when true , 4  ? LBRESULT;

state 19 :
to 20 : when true , 0 ! CLIENTRESULT;

state 17 :
to 18 : when true , 1 ! LBREPLY;

state 1 :
to 2 : when true , 1 ! LBREQUEST;

state 28 :
to 29 : when true , 0 ! CLIENTRESULT;

state 23 :
to 24 : when true , 4  ? LBCOMPUTE;

state 25 :
to 26 : when true , 4  ? VM1RES;
...
state 16 :
to 17 : when true , 4  ? VM2RESULT;

state 24 :
to 25 : when true , 2 ! VM1REQ;

state 14 :
to 16 : when true , 3 ! VM2REQUEST;
...
state 29 :
to 30 : when true , 4  ? CLIENTREQUEST;

state 2 :
to 3 : when true , 4  ? LBCOMPUTE;

state 20 :
to 21 : when true , 4  ? CLIENTREQUEST;
