...
state 30 :
to 31 : when true , 4  ? CLIENTREQUEST;

state 14 :
to 16 : when true , 4  ? LBCOMPUTE;

state 6 :
to 7 : when true , 1 ! LBREPLY;

state 15 :
to 17 : when true , 3 ! VM2REQUEST;
...
state 28 :
to 29 : when true , 4  ? LBRESULT;

state 23 :
to 24 : when true , 4  ? LBCOMPUTE;
...
state 24 :
to 25 : when true , 2 ! VM1REQ;

state 21 :
to 22 : when true , 4  ? CLIENTREQUEST;

state 10 :
to 12 : when true , 3 ! VM2LOGIN;
...
to 13 : when true , 4  ? LBCOMPUTE;

state 9 :
to 10 : when true , 4  ? CLIENTREQUEST;

state 13 :
to 15 : when true , 3 ! VM2LOGIN;

state 7 :
to 8 : when true , 4  ? LBRESULT;

state 29 :
to 30 : when true , 0 ! CLIENTRESULT;

state 2 :
to 3 : when true , 4  ? LBCOMPUTE;

state 20 :
to 21 : when true , 0 ! CLIENTRESULT;
