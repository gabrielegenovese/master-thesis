...
Let's consider Definition~\ref{def:deadlock-free}. The modified version for the
tool is the following.

\bigskip

\begin{definition}[Deadlock-free]\label{def:deadlock-free-tool}
A system of CFSMs $\cfsms$ is \emph{deadlock-free} in $\rscmodel$
if for every execution 
$e \in \executionsofcfsms{\acceptcompletion{P}}{}$,  
there exists an execution $e'\in \executionsofcfsms{P}{}$ 
with $e \prefixorder e'$, where $P=\mathrm{prod}_s(\cfsms)$.  
\end{definition}

%% nella tesi mettere la def di ppdp spiegata bene che è quella che
%% include tutte le semantiche di network
...
Informally, a given system satisfies \emph{progress} if, from every reachable node,
either the system can eventually perform a transition, or the node is a final one.
