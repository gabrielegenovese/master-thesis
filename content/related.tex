...
This thesis is centred around the study of the \emph{realisability 
...
the realisability problem, while also tracing the line of research 
back to early contributions such as Alur~et~al.~\cite{alur2000inference} 
and Lohrey~et~al.~\cite{lohrey2003s}.  
...
Alur~et~al.~\cite{alur2005s}, which also inspired part of our
...
informal definition of Alur~et~al.~\cite{alur2005s} and discuss
the differences.
...
remains decidable, and it is \verb|EXPSPACE|-cpmplete, but weak realisability 
becomes undecidable~\cite{alur2005s}. 
Later, Lohrey~et al.~\cite{lohrey2003s} proved, with a technique
...
for a specific kind of HMSCs, called globally cooperative. % TODO: capire cos'è globally cooperative
...
\begin{table}[!ht]
	...
	\begin{tabular}{|l|c|c|c|}
		...
		\hline
	\end{tabular}
	\caption{Summary of results on realisability in~\cite{alur2005s}.}
...
Projectability is, therefore, comparable to the realisability
...
several initiators, but it also makes the realisability problem 
...
showed that realisability is decidable by encoding global types 
...
sender-driven choice, with realisability shown to be in 
\verb|PSPACE|-the first precise complexity bound for this fragment.

We recall the definition of the \emph{Realisability Problem} as 
...
\cite{lanese2013amending}, which insert minimal extra communications 
to guarantee knowledge of choice.  

Conceptually, this problem closely resembles the \emph{sender-driven 
...
\section{Other works}
Stutz et al.~\cite{stutz2025automata} introduce \emph{Protocol State  
Machines} (PSMs), an automata-based formalism that generalises both  
MPST and HMSC. Their approach bears resemblance to choreography-automata  
models, specifying interactions via state machines over communication  
labels. Crucially, they investigate the computational complexity of  
type checking and realisability: when restricted to choice-free or  
single-choice fragments, checking realisability or well-formedness is  
decidable (often in PTIME), but once \emph{mixed choice} is permitted,  
the realisability problem becomes undecidable. This result lends further  
evidence to the hypothesis that unrestricted mixed-choice global types  
are intrinsically hard to implement in distributed systems.

Another relevant contribution is by Guanciale et al.~ 
\cite{guanciale2018realisability}, who study the 
