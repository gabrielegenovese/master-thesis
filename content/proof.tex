...
solution to an RPCP instance into the (non-)realizability of a
...
Assume $\Delta$ has a solution $R=(i_1,a_1,b_1,\ldots,i_m,a_m,b_m)$.
From this, construct two MSC sequences:
\[
M = M^{a_1}_{i_1}\cdots M^{a_m}_{i_m}, \qquad
M' = M^{b_1}_{i_1}\cdots M^{b_m}_{i_m}.
\]
Both $M,M'$ are synchronous concatenations of synchronous MSCs.
Consider now a third MSC $M''$ obtained by taking the projections
$M'|_1,M'|_2$ and $M|_3,M|_4$. Intuitively, processes $P_1,P_2$
...
$M''$ is acyclic and \emph{synchronous}. 
The $M''$ synchrony is not trivial, therefore we need to prove it.
This part was not present in the original proof.
By construction, $L^*$ weakly
implies $M''$, but $M'' \notin L^*$, since at least one tile
differs. Hence, $L^*$ is \emph{not realizable}.

\bigskip

\textbf{Step 3: Direction ``$\Leftarrow$''.}
Suppose $L^*$ is not realizable: then there exists some MSC $M^@$
...
\[
\Delta \in \text{RPCP} \quad\iff\quad L^* \text{ is not realizable}.
\]
Since RPCP is known to be \textbf{undecidable}, it follows that
\emph{checking weak realizability of a global type is also
undecidable}.
\end{proof}
